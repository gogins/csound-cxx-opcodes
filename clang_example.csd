...
class InvokableReverb : public ClangInvokableBase {
    ...
    public:
        ...
        int init(CSOUND *csound, OPDS *opds, MYFLT **outputs, MYFLT **inputs) override {
            ...
            result = ClangInvokableBase::init(csound, opds, outputs, inputs);
            MYFLT T60 = *(inputs[0]);
...
i_result clang_compile "reverb_main", S_reverb_code, "-v -g -O2 -std=c++14 -I/usr/local/include/csound  -I. -stdlib=libstdc++", "/usr/lib/gcc/x86_64-linux-gnu/9/libstdc++.so /usr/lib/gcc/x86_64-linux-gnu/9/libgcc_s.so /usr/local/lib/libstk.so /usr/lib/x86_64-linux-gnu/libm.so /usr/lib/x86_64-linux-gnu/libpthread.so"
...
k_t60 init 5.5
aleft, aright clang_invoke "reverb_factory", 3, k_t60, aleft_in, aright_in
...
std::uniform_real_distribution<> random_pan(.05, .95);
...
i_result clang_compile "score_generator", S_score_generator_code, "-v -g -O2 -std=c++14 -I/usr/local/include/csound -stdlib=libstdc++", "/usr/lib/gcc/x86_64-linux-gnu/9/libstdc++.so /usr/lib/gcc/x86_64-linux-gnu/9/libgcc_s.so /usr/lib/x86_64-linux-gnu/libm.so /usr/lib/x86_64-linux-gnu/libpthread.so"
